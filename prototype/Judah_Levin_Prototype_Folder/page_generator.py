...
# ============================================================================
def _template_path(brand, color, page_type):
    h = hashlib.md5(f"{brand}|{color}|{page_type}".encode()).hexdigest()[:10]
    return os.path.join(TEMPLATE_DIR, f"{page_type}_{h}.json")


def get_template(brand, color, page_type, key, catalog):
    """Return (template, in_tok, out_tok, from_cache)."""
    path = _template_path(brand, color, page_type)
    ...
    # The prompt itself carries ONLY the company-specific brief + output constraints;
    # all general "how to design well" content comes from the retrieved knowledge.
    prompt = (
        "=== COMPANY BRIEF (specific to this client) ===\n"
        f"Client: '{brand}'. Brand accent color: {color}.\n"
...
def render_product_page(tpl, brand, color, product):
...
# ============================================================================
def generate_store(brand, color, api_key, n_products):
    ...
    try:
        home_tpl, i0, o0, cached0 = get_template(brand, color, "home", key, catalog)
        prod_tpl, i1, o1, cached1 = get_template(brand, color, "product", key, catalog)
        chk_tpl, i2, o2, cached2 = get_template(brand, color, "checkout", key, catalog)
    ...
    # The savings math your team presents: optimized vs naive per-page generation.
...
def apply_edit(brand, color, api_key, section_id, instruction):
    key = (api_key or "").strip() or os.environ.get("MISTRAL_API_KEY", "")
    if not key:
        return "", "Paste a Mistral API key first."
    ...
    tpl, _, _, cached = get_template(brand, color, "product", key, catalog)
    if not cached:
    ...
    with open(_template_path(brand, color, "product"), "w", encoding="utf-8") as f:
        f.write(tpl.model_dump_json())
    TRACKER.add("diff edits", saved)

    html = render_product_page(tpl, brand, color, load_catalog()["products"][0])
    report = (
...
def list_sections(brand, color):
    path = _template_path((brand or "Demo Mart").strip(), (color or "#3B5998").strip(), "product")
...
def build_ui():
    with gr.Blocks(title="Storefront Generator") as demo:
        gr.Markdown("## AI storefront generator — token-optimized pipeline\n"
                    "Enterprise catalog in → ONE AI design call → every page rendered free.")
        with gr.Row():
            brand = gr.Textbox(label="Enterprise brand name", value="Demo Mart")
        ...
        with gr.Row():
            ...
            instruction = gr.Textbox(label="Edit instruction", placeholder="e.g. make the heading friendlier")
        edit_btn = gr.Button("Apply edit")

        gen_btn.click(generate_store, [brand, color, api_key, n_products], [preview, report])
        show_btn.click(list_sections, [brand, color], sections_md)
        edit_btn.click(apply_edit, [brand, color, api_key, section_id, instruction], [preview, report])
    return demo


if __name__ == "__main__":
    build_ui().launch(server_port=7861)
