...
def count_tokens(text: str) -> int:
    ...
    Falls back to a rough word-based estimate if tiktoken's encoding can't be
    loaded (e.g. no internet access on first use) — this is a measurement
    helper, so it should never be the thing that breaks the app."""
    global _enc
    text = text or ""
    try:
        if _enc is None:
